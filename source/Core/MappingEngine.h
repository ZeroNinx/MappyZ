...
private:
    // 判断输入事件是否匹配规则的输入条件
    ZERO_NODISCARD bool DoesRuleMatchInput(const SInputEvent& Event, const SMappingRule& Rule) const;

    // 根据匹配的规则和输入事件构建输出动作
    ZERO_NODISCARD TOptional<SAction> BuildAction(const SInputEvent& Event, const SMappingRule& Rule) const;

    // PressRelease 模式的动作构建
    ZERO_NODISCARD TOptional<SAction> BuildPressReleaseAction(const SInputEvent& Event, const SMappingRule& Rule) const;

    // Analog 模式的动作构建
    ZERO_NODISCARD TOptional<SAction> BuildAnalogAction(const SInputEvent& Event, const SMappingRule& Rule) const;
};
